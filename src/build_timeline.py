...
def build_timeline(filename, timeline_name):
            ...
            for other_index, other_row in df.iterrows():
                ...
                if other_index != index:
                    ...
                    if dependency_id == str(other_row['id']):
                        ...
                        relative_start_date = calculate_relative_start_date(other_row['start_date'], other_row['duration'],partial_starts[idx], units)
    ...
    for index, row in df.iterrows():
        ...
        end_date = calculate_relative_start_date(row['start_date'], row['duration'], 100, units)
        item_end_date = {
            'year': end_date.year,
        ...
        if pd.isna(row['duration']):
            ...
            item_duration = duration_to_milliseconds(row['duration'], units) #duration
    ...
    person_days = calculate_person_days(df, units)
    ...
    text_file = open("timeline_input.js", "w")
...
def duration_to_milliseconds(duration, units):
    if units == 'days':
...
def calculate_relative_start_date(dt, duration, partial_start_percentage, units):
...
def calculate_person_days(df, units):
    #for each task
    person_days = 0
    for index, row in df.iterrows():
